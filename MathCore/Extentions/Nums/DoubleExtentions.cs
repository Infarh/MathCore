...
namespace System
{
    ...
    public static class DoubleExtentions
    {
        ...
        public static double Sqrt(this double x)
        {
            ...
            Contract.Ensures(Contract.Result<double>() * Contract.Result<double>() == x);
            return double.IsNaN(x) ? double.NaN : Math.Sqrt(x);
        }
        ...
        {
            Contract.Requires(x != 0d);
            Contract.EnsuresOnThrow<DivideByZeroException>(x == 0d);
            return 1 / x;
        }
        ...
        {
            Contract.Requires(x > 0);
            return double.IsNaN(x) ? double.NaN : 20 * Math.Log10(x);
        }
        ...
        /// <returns>Значение в децебеллах</returns>
        [DST, Pure]
        public static double In_dB_byPower(this double x)
        {
            Contract.Requires(x > 0);
            return double.IsNaN(x) ? double.NaN : 10 * Math.Log10(x);
        }
        ...
        [DST, Pure]
        public static double From_dB(this double db)
        {
            Contract.Ensures(Contract.Result<double>() > 0);
            return double.IsNaN(db) ? double.NaN : Math.Pow(10, db / 20);
        }
        ...
        [DST, Pure]
        public static double From_dB_byPower(this double db)
        {
            Contract.Ensures(Contract.Result<double>() > 0);
            return double.IsNaN(db) ? double.NaN : Math.Pow(10, db / 10);
        }
        ...
        {
            Contract.Ensures(Contract.Result<double>() == deg * Consts.Geometry.ToRad);
            return deg * Consts.Geometry.ToRad;
        }
        ...
        {
            Contract.Ensures(Contract.Result<double>() == rad * Consts.Geometry.ToDeg);
            return rad * Consts.Geometry.ToDeg;
        }
    }
}