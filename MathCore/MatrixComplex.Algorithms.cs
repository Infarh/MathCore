        ...
        public static partial class Array
        {
            ...
            public static void Inverse([NotNull] Complex[,] matrix, [NotNull] Complex[,] result)
            {
                ...
                for (var i = 1; i < N; i++)
                {
                    ...
                    Operator.Multyly(res, transvection, multiply_result);
            ...
            public static bool AreEquals([CanBeNull] Complex[,] A, [CanBeNull] Complex[,] B)
            {
                ...
                if (ReferenceEquals(B, null) || ReferenceEquals(A, null)) return false;
            ...
            {
                Complex sqr(Complex a) => a * a;

                Complex pythag(Complex a, Complex b)
                {
                    var absa = a.Abs;
                    var absb = b.Abs;
                    if (absa > absb) return absa * Complex.Real + sqr(Math.Sqrt(absb / absa));
                    return absb.Equals(0) ? default : absb * Complex.Real + sqr(Math.Sqrt(absa / absb));
                }

                Complex sign(Complex a, Complex b) => b.Abs >= 0 ? a.Abs : -a.Abs;
                ...
                for (var i = 0; i < M; i++)
                {
                    ...
                    Complex max(Complex z, double x) => z.Abs > x ? z : x;
            ...
            public static partial class Operator
            {
                ...
                public static Complex Multyply([NotNull] Complex[] v1, [NotNull] Complex[] v2)
                {
                ...
                public static Complex[] Multyply([NotNull] Complex[] v1, Complex v2)
                {
                    if (v1 is null) throw new ArgumentNullException(nameof(v1));
                ...
                /// <exception cref="ArgumentNullException">В случае если <paramref name="matrix"/> не определена</exception>
                [NotNull]
                public static Complex[,] Multyply([NotNull] Complex[,] matrix, Complex x)
                {
                    if (matrix is null) throw new ArgumentNullException(nameof(matrix));
                    Contract.Ensures(Contract.Result<Complex[,]>() != null);
                    Contract.Ensures(Contract.Result<Complex[,]>().GetLength(0) == matrix.GetLength(0));
                ...
                /// <exception cref="ArgumentNullException">В случае если <paramref name="B"/> не определена</exception>
                [NotNull]
                public static Complex[,] Multyly([NotNull] Complex[,] A, [NotNull] Complex[,] B)
                {
                    if (A == null) throw new ArgumentNullException(nameof(A));
                    if (B == null) throw new ArgumentNullException(nameof(B));
                ...
                public static void Multyly([NotNull] Complex[,] A, [NotNull] Complex[,] B, [NotNull] Complex[,] result)
                {
                    if (A == null) throw new ArgumentNullException(nameof(A));
                    if (B == null) throw new ArgumentNullException(nameof(B));
                ...
                /// <exception cref="ArgumentException">В случае если размерности матриц несогласованы</exception>
                [NotNull]
                public static Complex[,] Divade([NotNull] Complex[,] A, [NotNull] Complex[,] B) => Multyly(A, Inverse(B, out var _));
                ...
                public static Complex[,] Concatinate([NotNull] Complex[,] A, [NotNull] Complex[,] B)
                {
                    ...
                    {
                        result = new Complex[A_N + B_N, A_M];
                        for (var i = 0; i < A_N; i++) for (var j = 0; j < A_M; j++) result[i, j] = A[i, j];
                        var i0 = A_N;
                        for (var i = 0; i < B_N; i++) for (var j = 0; j < B_M; j++) result[i + i0, j] = B[i, j];
                    }
                    else if (A_N == B_N) //Конкатинация по строкам
                    {
                        result = new Complex[A_N, A_M + B_M];
                        for (var i = 0; i < A_N; i++) for (var j = 0; j < A_M; j++) result[i, j] = A[i, j];
                        var j0 = A_M;
                        for (var i = 0; i < B_N; i++) for (var j = 0; j < B_M; j++) result[i, j + j0] = B[i, j];
                    }
