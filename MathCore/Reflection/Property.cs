...
public class Property<TObject, TValue> : IValue<TValue>
{
    ...
    protected virtual void OnValueChanged(EventArgs? E = null) => ValueChanged.Start(this, E ?? EventArgs.Empty);
    ...
    private void Initialize(TObject? o, string PropertyName, bool IsPrivate)
    {
        if (_Descriptor != null
            && _Object != null
    ...
    public override string ToString()
    {
        ...
        //    SupportsChangeEvents ? "E" : string.Empty);
        return new StringBuilder(property_type.Length + 8 + host.Length + _Name.Length + value.Length)
           .Append(property_type)
           .Append('(').Append(CanRead, 'R').Append(CanWrite, 'W').Append(SupportsChangeEvents, 'E').Append("): ")
           .Append(host).Append('.').Append(_Name).Append(value)
           .ToString();
    }
