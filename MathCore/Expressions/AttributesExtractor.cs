...
/// <remarks>Инициализация нового экземпляра <see cref="AttributesExtractor"/></remarks>
/// <param name="Info">Мета-информация о методе, для для которого надо осуществлять поиск атрибутов</param>
public class AttributesExtractor(MemberInfo Info)
{
    ...
    /// <returns>Атрибут, с указанным именем, либо <see langword="null"/></returns>
    public Attribute? this[string Name, bool IsInherit] => GetAttributes(Name, IsInherit).FirstOrDefault();
    ...
    public IEnumerable<Attribute> GetAttributes(string Name, bool IsInherit) =>
        Info.GetCustomAttributes(IsInherit)
           .Cast<Attribute>()
