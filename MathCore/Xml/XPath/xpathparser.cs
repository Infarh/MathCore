...
namespace System.Xml.XPath
{
    ...
    internal class XPathParser
    {
        ...
        private static bool IsNodeType([NotNull] XPathScanner Scanner) =>
        ...
        private static bool IsPrimaryExpr([NotNull] XPathScanner scanner) =>
        ...
        {
            var operand = ParseLocationPathPattern(QyInput);

            do
            {
                if (_Scanner.Kind != XPathScanner.LexKind.Union) return operand;
                NextLex();
                operand = new Operator(Operator.Op.Union, operand, ParseLocationPathPattern(QyInput));
            } while (true);
        }
            ...
            switch (_Scanner.Kind)
            {
                ...
                case XPathScanner.LexKind.Name:
                    if (_Scanner.CanBeFunction)
                        if ((operand = ParseIdKeyPattern(QyInput)) != null)
                            switch (_Scanner.Kind)
                            {
                                case XPathScanner.LexKind.Slash:
                                    NextLex();
