...
internal sealed class StringFunctions : Query
{
    ...
    {
        var str1 = ((Query)_ArgList[0]!).GetValue(reader)!.ToString().NotNull();
        var str2 = ((Query)_ArgList[1]!).GetValue(reader)!.ToString().NotNull();
    ...
    private string Substring(XPathReader reader)
    {
        ...
        if (_ArgList.Count == 3)
        {
            ...
            var max_length = str1.Length - num;
            if (num1 > max_length) num1 = max_length;
    ...
    private double StringLength(XPathReader reader) =>
        _ArgList is { Count: > 0 }
            ? ((Query)_ArgList[0]!).GetValue(reader)!.ToString()?.Length ?? 0
            : 0;

    private string Normalize(XPathReader reader)
    {
        var str1 = _ArgList is { Count: > 0 }
            ? ((Query)_ArgList[0]!).GetValue(reader)!.ToString()?.Trim() ?? string.Empty
            : string.Empty;
        var count = 0;
    ...
    private string Translate(XPathReader reader)
    {
        var str1 = ((Query)_ArgList[0]!).GetValue(reader)!.ToString().NotNull();
        var str2 = ((Query)_ArgList[1]!).GetValue(reader)!.ToString().NotNull();
        var str3 = ((Query)_ArgList[2]!).GetValue(reader)!.ToString().NotNull();
