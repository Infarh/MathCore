...
public class LambdaComparer<T>(Comparison<T?> Comparison) : IComparer<T>, IComparer
{
    ...
    /// <returns>
    /// Значение Условие Меньше нуля <paramref name="x"/> меньше, чем <paramref name="y"/>. 
    /// Нуль <paramref name="x"/> равно <paramref name="y"/>.
    /// Больше нуля<paramref name="x"/> больше, чем <paramref name="y"/>.
    /// </returns>
