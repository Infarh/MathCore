...
public class MD5 : HashAlgorithm
{
    ...
    private static void SetLength(Span<byte> buffer64, ulong length)
    {
        buffer64[^8] = (byte)(length << 3);
        buffer64[^7] = (byte)(length >> 5);
        buffer64[^6] = (byte)(length >> 13);
    ...
    //    Span<uint> result = stackalloc[] { 0x67452301U, 0xefcdab89U, 0x98badcfeU, 0x10325476U };

    //    Span<byte> buffer64 = stackalloc byte[64];
    //    var words = buffer64.Cast<uint>();
    ...
    //        if (data.Length >= 64)
    //        {
    //            Compute(input[..64].Cast<uint>(), ref result[0], ref result[1], ref result[2], ref result[3]);
    //            input = input[64..];
    //        }
    //        else
    //        {
    //            input.CopyTo(buffer64);
    //            buffer64[input.Length] = 0x80;

    //            if (64 - input.Length > 8)
    //            {
    //                SetLength(buffer64, length);
    //                Compute(words, ref result[0], ref result[1], ref result[2], ref result[3]);
    //                return result.CastToByte().ToArray();
    //            }
    ...
    public static byte[] Compute(byte[] data)
    {
        ...
        data.CopyTo(buffer64, 0);
        buffer64[length] = 0x80;

        SetLength(buffer64, (ulong)length);

        for (var i = 0; i < buffer64_length; i += 64)
            Compute(buffer64.AsSpan(i, 64).Cast<uint>(), ref result[0], ref result[1], ref result[2], ref result[3]);
    ...
    public static byte[] Compute(byte[] data)
    {
        uint[] result = { 0x67452301U, 0xefcdab89U, 0x98badcfeU, 0x10325476U };
...
#if NET8_0_OR_GREATER
    private static void Compute(Span<uint> buffer16, ref uint a0, ref uint b0, ref uint c0, ref uint d0)
    {
        var (A, B, C, D) = (a0, b0, c0, d0);
