...
public class Streebog
{
    ...
    [
        0, 08, 16, 24, 32, 40, 48, 56,
        1, 09, 17, 25, 33, 41, 49, 57,
    ...
    public Streebog(int OutputLength)
    {
        switch (OutputLength)
        {
            case 512:
            ...
            case 256:
                ...
                Array.Fill(_Iv, (byte)0x01);
#else
                for (var i = 0; i < 64; i++) _Iv[i] = 0x01;
#endif
                break;

            default:
                throw new ArgumentException("OutputLength должно быть либо 256, либо 512", nameof(OutputLength));
        }

        _OutLen = OutputLength;
    }
    ...
    private byte[] AddModulo512(byte[] a, byte[] b)
    {
        ...
        var temp_a = new byte[64];
        var temp_b = new byte[64];

        Array.Copy(a, 0, temp_a, 64 - a.Length, a.Length);
        Array.Copy(b, 0, temp_b, 64 - b.Length, b.Length);

        for (var i = 63; i >= 0; i--)
        {
            t = temp_a[i] + temp_b[i] + (t >> 8);
            temp[i] = (byte)(t & 0xFF);
        }
    ...
    public byte[] Compute(byte[] message)
    {
        ...
        {
            inc++;
            var temp_mes = new byte[64];
            Array.Copy(message, message.Length - inc * 64, temp_mes, 0, 64);
        ...
        {
            for (var i = 0; i < 64 - message1.Length - 1; i++)
                padded_mes[i] = 0;

            padded_mes[64 - message1.Length - 1] = 0x01;
            Array.Copy(message1, 0, padded_mes, 64 - message1.Length, message1.Length);
        }

        h = G_n(_N, h, padded_mes);
        var mes_len = BitConverter.GetBytes(message1.Length * 8);

        _N = AddModulo512(_N, mes_len.AsEnumerable().Reverse().ToArray());
        _Sigma = AddModulo512(_Sigma, padded_mes);
