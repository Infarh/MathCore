...
namespace MathCore
{
    ...
    public class CommandLineArgs
    {
        ...
        public IEnumerable<string> this[string key] => _Args.Where(v => v.Key == key).SelectMany(v => EnumValues(v.Value));
        ...
        public CommandLineArgs(string[] args)
        {
            ...
            var i = 0;
            for (; i < count; i++)
                if (_ArgsSrc[i].StartsWith("-") || _ArgsSrc[i].StartsWith("/"))
                {
                    _StartArgs = (0, i);
                    break;
                }
            ...
            for (; i < count; i++)
            {
                ...
                for (var j = i + 1; j < _ArgsSrc.Length; j++)
                    if (_ArgsSrc[j].StartsWith("-") || _ArgsSrc[j].StartsWith("/"))
                    {
