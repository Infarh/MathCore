...
public static class AdamsBashforth
{
        ...
        double t0 = 0)
    {
        ...
        if (N > 1) Y[1] = Step0(f, t0, dt, y0);
        if (N > 2) Y[2] = Step1(f, t0 + dt, dt, Y[1], y0);
        if (N > 3) Y[3] = Step2(f, t0 + 2 * dt, dt, Y[2], Y[1], y0);
        if (N > 4) Y[4] = Step3(f, t0 + 3 * dt, dt, Y[3], Y[2], Y[1], y0);
        if (N > 5) Y[5] = Step4(f, t0 + 4 * dt, dt, Y[5], Y[3], Y[2], Y[1], y0);
        for (var (i, t) = (6, t0 + 5 * dt); i < N; i++, t += dt)
            Y[i] = Step4(f, t, dt, Y[i - 1], Y[i - 2], Y[i - 3], Y[i - 4], Y[i - 5]);
